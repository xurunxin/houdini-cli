...
program.name(metadata.bin).version('0.1.0').description(`${metadata.label} 按需 CLI：发现工具、调用 MCP、启动应用、初始化环境和安装项目 skills。`)
...
common(app.command('launch').alias('start').description('启动对应应用及本次会话的桥接服务'))
  ...
  .action(wrap(async (_, command) => { const { config, options } = await context(command); options.timeout ??= 60000; return launch(config, options); }));
...
program.addHelpText('after', `\n示例：\n  ${metadata.bin} doctor\n  ${metadata.bin} setup --dry-run\n  ${metadata.bin} setup\n  ${metadata.bin} app launch\n  ${metadata.bin} tools list\n  ${metadata.bin} tools inspect <工具名>\n  ${metadata.bin} tools call <工具名> --args-file args.json\n  ${metadata.bin} skills install --target .\n\n业务结果为 JSON stdout；日志为 stderr。退出码：0 成功，1 运行/工具/环境错误，2 参数错误。\nCLI 按次启动和关闭 MCP；应用及其本机桥接单独运行。调用不自动重试。\n状态目录可通过 --home 或 ${metadata.id.toUpperCase().replaceAll('-', '_')}_HOME 指定。`);
try { await program.parseAsync(); }
  ...
  if (e.code === 'commander.helpDisplayed' || e.code === 'commander.version') process.exitCode = 0;
    ...
    process.exitCode = e.code?.startsWith('commander.') || e.code?.startsWith('INVALID_') ? 2 : 1;
  }
}
