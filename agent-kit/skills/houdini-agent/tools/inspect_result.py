...
def inspect(payload: object) -> dict:
    ...
    def walk(obj: object, path: str, depth: int = 0):
        ...
        if obj.get('ok') is True or obj.get('success') is True or state in ('success','ok','completed'):
        ...
        if 'structuredContent' in obj:
            walk(obj['structuredContent'],path+'.structuredContent',depth+1)
        if 'result' in obj:
            walk(obj['result'],path+'.result',depth+1)
        if isinstance(obj.get('results'), list):
            for i, val in enumerate(obj['results']): walk(val,f'{path}.results[{i}]',depth+1)
            ...
            for i, item in enumerate(obj['content']):
                ...
                if isinstance(text,str) and marker in text:
                    ...
                    try:
                        ...
                        walk(decoded, f'{path}.content[{i}].agent_result',depth+1)
                        if '--- Stderr ---' in text:
                ...
                try: parsed = json.loads(text)
                    ...
                    if isinstance(text,str) and text.strip().lower().startswith(('error:', 'error (', 'connection error', 'server error', 'traceback (', 'failed:')):
                        errors.append(f'{path}.content[{i}]: '+text[:1000])
                    else: parse_unknown.append(f'{path}.content[{i}]: free text not interpreted')
                else: walk(parsed, f'{path}.content[{i}]',depth+1)
